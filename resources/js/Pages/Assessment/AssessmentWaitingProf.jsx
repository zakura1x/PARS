...
const AssessmentWaitingProf = ({ assessment, initialWaitingStudents, assessmentCode }) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      fetch(`/api/assessment/${assessment.id}/waiting-students`)
  ...
  const startAssessment = async () => {
    setIsLoading(true)
    try {
      const response = await fetch(`/api/assessment/start/${assessment.id}`, { method: "POST" })
    ...
    <div className="card w-full max-w-4xl mx-auto bg-base-100 shadow-xl">
      <div className="card-body">
        ...
        <div className="flex items-center space-x-2 mb-4">
          <span className={`badge ${assessment.status === "Waiting" ? "badge-secondary" : "badge-primary"}`}>
            {assessment.status}
          </span>
          <button className="btn btn-outline btn-sm" onClick={copyAssessmentCode}>
        ...
        {waitingStudents.length > 0 ? (
          ...
          <p className="text-base-content opacity-70">No students are currently waiting.</p>
        )}
        <div className="card-actions justify-end mt-6">
          <button className="btn btn-ghost" onClick={() => window.history.back()}>
            Cancel
          </button>
          <button className="btn btn-primary" onClick={startAssessment} disabled={isLoading}>
